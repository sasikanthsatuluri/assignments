 '''
A Python variable is a reserved memory location to store values. str, num, are all valid name for the variables.The name of the variable cannot start with a number.
 Variable name is case sensitive in Python which means num and NUM are two different variables in python is None.
 
 example : x = 5
           y = "John"
           print(x)
           print(y)
here x and y are variables storing an integer and a string individually. printing these variables gives the elements stored in them.
# Python Variables:
